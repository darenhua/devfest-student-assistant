...
const API_BASE = "/modules/reverse-forward-overlay/api";
...
async function apiSaveManifest(entries: ManifestEntry[]) {
  return apiFetch<{ manifest: { createdAt: string; entries: ManifestEntry[] }; branches: PrototypeBranch[] }>(
    "/save-manifest",
    {
      method: "POST",
...
// ---------------------------------------------------------------------------
// Next-step action panel (stage-specific)
// ---------------------------------------------------------------------------
...
function ImportTransformZone({
  ...
  async function handleImport() {
    try {
      ...
      if (!raw) {
        setError("No annotations found in sessionStorage. Use react-grab to comment on components first.");
        return;
      }
      ...
      if (commentItems.length === 0) {
        setError("No comment annotations found. Use react-grab to add comments to components.");
        return;
      }
        ...
        <>
          <div className="space-y-3">
            {entries.map((entry, i) => (
              ...
              >
                ...
                {/* File path (editable if unresolvable) */}
                {!entry.componentPath && (
          ...
          <div className="flex items-center justify-between border-t border-zinc-800 pt-3">
            <p className="text-xs text-zinc-500">
              {enabledCount}/{entries.length} components selected →{" "}
...
export default function ReverseForwardOverlayPage() {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 mt-0.5">
                Overlay Annotations → Extraction Specs → Prototype Branches
              </p>
            </div>
            <button
              onClick={refresh}
      ...
      <div className="mx-auto max-w-5xl px-6 pb-10">
        <details className="group">
          <summary className="cursor-pointer text-xs text-zinc-600 hover:text-zinc-400 transition-colors">
            Pipeline stages reference (REV+FWD — 5 stages)
          </summary>
          <div className="mt-3">
